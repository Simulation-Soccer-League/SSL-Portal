...
ui <- function(id) {
  ...
  shiny$bootstrapPage(
    ...
    router_ui(
      ...
      route("tracker/budget", budget$ui(ns("budget"))),
...
server <- function(id) {
  shiny$moduleServer(id, function(input, output, session) {
    ...
    shiny$observe({
      ...
      } else if (current == "tracker/budget" & !loadedServer$budget) {
        
        budget$server("budget")
        loadedServer$budget <- TRUE
