  ...
  shiny$moduleServer(id, function(input, output, session) {
    ...
    output$ui <- shiny$renderUI({
      shiny$tagList(
        bslib$card(
          bslib$card_header(
            shiny$h3("Add New Contract")
          ),
          bslib$card_body(
            shiny$p(
              "This page is used to add a player's contract after they have accepted an offer on the forum."
            ),
            bslib$layout_column_wrap(
              width = 1 / 2,
              shiny$div(
                shiny$h5("Player Information"),
                shiny$selectInput(
                  ns("selectedPlayer"),
                  "Select a player", 
                  choices = NULL
                ...
                shiny$div(
                  shiny$h5("Organization Information"),
                  shiny$selectInput(
                    ns("organization"),
                    "Select the organization making the signing",
                    choices = NULL
                  ),
                  shiny$selectInput(
                    ns("affiliate"),
                    "Select affiliate the player is signed to",
                    choices = c("Major" = 1, "Minor" = 2)
                  )
                )
              ),
              shiny$div(
                shiny$h4("New Contract Information"),
                shiny$textInput(
                  ns("url"),
                  "Link",
                  value = ""
                ...
                shiny$selectInput(
                  ...
                  choices = c("FA", "IFA", "DRAFT", "EXTENSION")
                ),
                bslib$layout_column_wrap(
                  width = 1 / 2,
                  shiny$numericInput(
